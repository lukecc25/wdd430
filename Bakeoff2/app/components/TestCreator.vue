...
async function handleSubmit(event) {
  ...
  try {
    ...
    const data = await authedFetch('/api/lessons', {
      method: 'POST',
      body: payload,
    });

    emit('created', data.lesson);
  } catch (err) {
    const msg = err?.data?.message || err?.statusMessage || err?.message || 'Could not save lesson';
    emit('error', msg);
  } finally {
    saving.value = false;
  }
    ...
    <form class="test-creator__form" @submit="handleSubmit">
        ...
        <div class="test-creator__options">
          ...
          >
            <input
              ...
              :value="oi"
            />
            <input v-model="question.options[oi]" type="text" required :placeholder="`Option ${oi + 1}`" />
          </label>
        </div>
      ...
      <div class="test-creator__actions">
        ...
        <button type="submit" class="test-creator__submit" :disabled="saving">
          {{ saving ? 'Saving…' : form.visibility === 'public' ? 'Publish lesson' : 'Save private lesson' }}
        </button>
      </div>
    </form>
  </div>
